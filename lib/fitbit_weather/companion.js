...
// export const findLocationByAddress = (locationConfig) => {
//   const { addressLine, adminDistrict, countryRegion, postalCode } = locationConfig;

//   const baseURL = 'https://dev.virtualearth.net/REST/v1/Locations';

//   const url = `${baseURL}?key=${conf.bingMapKey}&countryRegion=${countryRegion}
//     &addressLine=${addressLine}&postalCode=${postalCode}&adminDistrict=${adminDistrict}`;

//   const errorResponse = {
//     error: true,
//     message: 'Cound not find the specified location.',
//   };
...
//       const firstSet = resourceSets[0];

//       if (!firstSet) { return errorResponse; }

//       const resource = firstSet.resources[0];

//       if (!resource) { return errorResponse; }
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(encodeURI(url))
      ...
      .then(data => {
        ...
        //data.weather[0].description.split('').length() == 2 ) ? lines = 2 : lines = 1;          
        ...
        const weather = {
          ...
          //lines == 2 ? 
            //description: data.weather[0].description.split('')[0], description2: data.weather[0].description.split('')[1], : description: data.weather[0].description,
